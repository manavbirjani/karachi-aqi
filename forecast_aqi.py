...
# Get latest pollutant averages (fallback 0 if missing)
avg_pm25 = features_df.get("pm25", pd.Series([0])).mean()
avg_pm10 = features_df.get("pm10", pd.Series([0])).mean()
avg_o3 = features_df.get("o3", pd.Series([0])).mean()
...
# Forecast next 3 days, every 3 hours
forecast_data = []
now = datetime.now()

for i in range(24):  # 24 intervals of 3 hours = 3 days
...
# Match exact order of model's features
